...
const JobApiEXP = () => {
    ...
    const filterData = () => {
        const filterBlogs = expdata.filter((job) => {
            return job?.title && job.title.some((eachData) =>
    ...
    const handleLocation = (city) => {
        if (selectloc.includes(city)) {
            setSelectLoc((expfilter) => expfilter.filter(item => item !== city));

        } else {
            setSelectLoc((expfilter) => [...expfilter, city])
        }
    };
                ...
                <div className="row">
                    <div className="col-lg-10">
                        ...
                        <div className="locationsGrid">
                            ...
                            {expfilter.map((blog) => (
                                <div key={blog._id} onClick={(e) => handleLocation(blog.title)} className="" >
                                    <div className="mt-5">
                                        <div className="">
                                            <div className="">
                                                <button className="btn  " style={{ border: '1px solid black', width: '280px' }}>{blog.title}</button>
                                            </div>
